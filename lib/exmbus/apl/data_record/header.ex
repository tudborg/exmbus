...
defmodule Exmbus.Apl.DataRecord.Header do
  ...
  defmodule InvalidHeader do
    defstruct [
      dib: nil,
      vib: nil,
      error_message: nil, # human friendly reason why this header was invalid
    ]
  end
      ...
      {:ok, [%DIB{}=dib | _]=inner_ctx, <<>>} ->
        ...
        case VIB.parse(vib_bytes, opts, inner_ctx) do
          ...
          {:error, {:invalid, reason}, <<>>} ->
            {:ok, [%InvalidHeader{dib: dib, vib: nil, error_message: "VIB invalid with reason #{inspect reason}"} | ctx], rest_after_vib}
        end
      {:error, reason, <<>>} ->
        {:ok, [%InvalidHeader{error_message: "Parsing DIB failed: #{inspect reason}"} | ctx], rest_after_dib}
    end
  end
