defmodule Exmbus.Parser.Apl.DataRecord.ValueInformationBlock.VifTableMain do
  ...
  def parse(<<e::1, 0b1111100::7, rest::binary>>, ctx) do
    ...
    case more(e, rest, ctx, :plain_text_unit) do
      ...
      {:ok, %VIB{description: :plain_text_unit}=vib, <<len, rest::binary>>} ->
        ...
        # The right-most character is transmitted first, so we need to reverse the
  ...
  defp _unparse(%VIB{extensions: [], description: :energy,                 multiplier: m, unit: "Wh"}),      do: {:ok, <<0::1, 0b0000::4,  (log10int(m)+3)::3>>}
  defp _unparse(%VIB{extensions: [], description: :energy,                 multiplier: m, unit: "J"}),       do: {:ok, <<0::1, 0b0001::4,  (log10int(m)+0)::3>>}
  defp _unparse(%VIB{extensions: [], description: :volume,                 multiplier: m, unit: "m^3"}),     do: {:ok, <<0::1, 0b0010::4,  (log10int(m)+6)::3>>}
  defp _unparse(%VIB{extensions: [], description: :mass,                   multiplier: m, unit: "kg"}),      do: {:ok, <<0::1, 0b0011::4,  (log10int(m)+6)::3>>}
  defp _unparse(%VIB{extensions: [], description: :on_time,                               unit:  u}),        do: {:ok, <<0::1, 0b01000::5, (encode_time_unit(u))::2>>}
  defp _unparse(%VIB{extensions: [], description: :operating_time,                        unit:  u}),        do: {:ok, <<0::1, 0b01001::5, (encode_time_unit(u))::2>>}
  defp _unparse(%VIB{extensions: [], description: :power,                  multiplier: m, unit: "W"}),       do: {:ok, <<0::1, 0b0101::4,  (log10int(m)+3)::3>>}
  defp _unparse(%VIB{extensions: [], description: :power,                  multiplier: m, unit: "J/h"}),     do: {:ok, <<0::1, 0b0110::4,  (log10int(m)+0)::3>>}
  defp _unparse(%VIB{extensions: [], description: :volume_flow,            multiplier: m, unit: "m^3/h"}),   do: {:ok, <<0::1, 0b0111::4,  (log10int(m)+6)::3>>}
  defp _unparse(%VIB{extensions: [], description: :volume_flow_ext,        multiplier: m, unit: "m^3/min"}), do: {:ok, <<0::1, 0b1000::4,  (log10int(m)+7)::3>>}
  defp _unparse(%VIB{extensions: [], description: :volume_flow_ext,        multiplier: m, unit: "m^3/s"}),   do: {:ok, <<0::1, 0b1001::4,  (log10int(m)+9)::3>>}
  defp _unparse(%VIB{extensions: [], description: :mass_flow,              multiplier: m, unit: "kg/h"}),    do: {:ok, <<0::1, 0b1010::4,  (log10int(m)+3)::3>>}
  defp _unparse(%VIB{extensions: [], description: :flow_temperature,       multiplier: m, unit: "°C"}),      do: {:ok, <<0::1, 0b10110::5, (log10int(m)+3)::2>>}
  defp _unparse(%VIB{extensions: [], description: :return_temperature,     multiplier: m, unit: "°C"}),      do: {:ok, <<0::1, 0b10111::5, (log10int(m)+3)::2>>}
  defp _unparse(%VIB{extensions: [], description: :temperature_difference, multiplier: m, unit: "K"}),       do: {:ok, <<0::1, 0b11000::5, (log10int(m)+3)::2>>}
  defp _unparse(%VIB{extensions: [], description: :external_temperature,   multiplier: m, unit: "°C"}),      do: {:ok, <<0::1, 0b11001::5, (log10int(m)+3)::2>>}
  defp _unparse(%VIB{extensions: [], description: :pressure,               multiplier: m, unit: "bar"}),     do: {:ok, <<0::1, 0b11010::5, (log10int(m)+3)::2>>}
  defp _unparse(%VIB{extensions: [], description: :date,           coding: :type_g}),                        do: {:ok, <<0::1, 0b1101100::7>>}
  defp _unparse(%VIB{extensions: [], description: :time,           coding: :type_j}),                        do: {:ok, <<0::1, 0b1101101::7>>}
  defp _unparse(%VIB{extensions: [], description: :naive_datetime, coding: :type_f}),                        do: {:ok, <<0::1, 0b1101101::7>>}
  defp _unparse(%VIB{extensions: [], description: :naive_datetime, coding: :type_i}),                        do: {:ok, <<0::1, 0b1101101::7>>}
  defp _unparse(%VIB{extensions: [], description: :datetime,       coding: :type_m}),                        do: {:ok, <<0::1, 0b1101101::7>>}
  defp _unparse(%VIB{extensions: [], description: :units_for_hca}),                                          do: {:ok, <<0::1, 0b1101110::7>>}
  defp _unparse(%VIB{extensions: [], description: :averaging_duration, unit: u}),                            do: {:ok, <<0::1, 0b11100::5, (encode_time_unit(u))::2>>}
  defp _unparse(%VIB{extensions: [], description: :actuality_duration, unit: u}),                            do: {:ok, <<0::1, 0b11101::5, (encode_time_unit(u))::2>>}
  defp _unparse(%VIB{extensions: [], description: :fabrication_no}),                                         do: {:ok, <<0::1, 0b1111000::7>>}
  defp _unparse(%VIB{extensions: [], description: :enhanced_identification}),                                do: {:ok, <<0::1, 0b1111001::7>>}
  defp _unparse(%VIB{extensions: [], description: :address}),                                                do: {:ok, <<0::1, 0b1111010::7>>}

  defp _unparse(%VIB{extensions: es, description: :manufacturer_specific_encoding}) do
    vife_bytes =
      es
