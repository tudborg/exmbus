defmodule Exmbus.Parser.Apl do
  ...
  def parse(bin, opts, %{apl: nil} = ctx) do
    with {:ok, ctx, ""} <- maybe_handle_apl(bin, opts, ctx),
         {:ok, ctx} <- maybe_decrypt_apl(opts, ctx),
         {:ok, ctx} <- maybe_parse_apl(opts, ctx) do
      {:ok, ctx}
    end
  end

  defp maybe_handle_apl(bin, %{handle_apl: false}, ctx), do: {:ok, ctx, bin}
  defp maybe_handle_apl(bin, opts, ctx), do: Unparsed.parse(bin, opts, ctx)

  defp maybe_decrypt_apl(%{decrypt_apl: false}, ctx), do: {:ok, ctx}
  defp maybe_decrypt_apl(opts, ctx), do: Unparsed.decrypt(opts, ctx)

  defp maybe_parse_apl(%{parse_apl: false}, ctx), do: {:ok, ctx}
  defp maybe_parse_apl(opts, ctx), do: parse_apl(opts, ctx)

  # assume decrypted apl bytes as first argument, parse data fields
  # an return an {:ok, Apl+ctx}
  defp parse_apl(opts, %{tpl: %Tpl{frame_type: :full_frame}, apl: %{} = apl} = ctx) do
    # NOTE:
  ...
  defp parse_apl(opts, %{tpl: %Tpl{frame_type: :format_frame}, apl: %{} = apl} = ctx) do
    FormatFrame.parse(apl.plain_bytes, opts, ctx)
  end

  defp parse_apl(opts, %{tpl: %Tpl{frame_type: :compact_frame}, apl: %{} = apl} = ctx) do
