# Qundis Walk-by data unpacking

This is a snippet from a previous attempt at unpacking the walk-by data from
a Qundis HCA. (IIRC)
...
defmodule Qundis do
  ...
  defp find_walk_by(%{vif: "Qundis Walk-by", value: v}) when is_binary(v), do: true
  ...
  defp decrypt(parsed, <<0, 0x82, acc, status, 0x35, encrypted::binary>>, key) when is_binary(key) and bit_size(key) == 128 do
    bcd_serial = int_to_bcd(parsed.serial)
    iv = <<
  ...
  defp parse(parsed = %{}, walk_by) do
    ...
    case Parser.Fields.parse(fieldsstring, parsed) do
      {:error, message} ->
         {:error, message}
